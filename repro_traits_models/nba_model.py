...
nba_geno = np.loadtxt("C:/Users/Jaidyn/Documents/ASSURE/python/project/repro_traits_model/NBA/spg_NBA.raw", dtype = "float32", 
                      skiprows = 1, usecols = use_columns)
nba_pheno = np.loadtxt("C:/Users/Jaidyn/Documents/ASSURE/python/project/repro_traits_model/NBA/NBA_pheno.csv", dtype = "float32",
                       skiprows = 1, delimiter = ",", usecols = range(1,2))
nba_pheno = np.reshape(nba_pheno, (nba_pheno.shape[0], 1))

nba_geno = StandardScaler().fit_transform(nba_geno) 
nba_pheno = StandardScaler().fit_transform(nba_pheno)

#Check Geno and Pheno Shapes
print("nba geno:", nba_geno.shape)
print("nba pheno:", nba_pheno.shape)
...
#Datasets 
nba_train_dataset = GenDataset(nba_geno[0:train_percent,], nba_pheno[0:train_percent, 0])
nba_test_dataset = GenDataset(nba_geno[train_percent:,], nba_pheno[train_percent:, 0])

#Create Test Abs
nba_test_abs = int(len(nba_train_dataset) * 0.8)
nba_train_subset, nba_val_subset = torch.utils.data.random_split(nba_train_dataset, [nba_test_abs, len(nba_train_dataset) - nba_test_abs])
...
#DataLoaders
nba_train_loader = torch.utils.data.DataLoader(dataset = nba_train_subset, 
                                              batch_size = batch_size, 
                                              shuffle = True) 
nba_val_loader = torch.utils.data.DataLoader(dataset = nba_val_subset, 
                                         batch_size = batch_size,
                                         shuffle = False)
nba_test_loader = torch.utils.data.DataLoader(dataset = nba_test_dataset, 
                                          batch_size = batch_size, 
                                          shuffle = False)
...
#Model Training 
def train(model, nba_train_loader, nba_val_loader, criterion, optimizer, num_epochs): 
    best_val_acc = -1.0 
    patience = 10
    counter = 0 
    all_acc = [] 
    ...
    for epoch in range(num_epochs): 
            ...
            for geno, pheno in nba_train_loader: 
                geno = geno.to(device)
            ...
            with torch.no_grad(): 
                for geno, pheno in nba_val_loader: 
                    geno = geno.to(device) 
                    pheno = pheno.to(device)
...
#Model Testing 
def test(model, nba_test_loader): 
    with torch.no_grad(): 
        outputs_list = [] 
        pheno_list = [] 
        for geno, pheno in nba_test_loader:
            geno = geno.to(device)
            pheno = pheno.to(device)
...
def train_with_cv(model, criterion, optimizer, X, y, num_epochs, batch_size, n_splits = 5): 
    ...
    shuffle_indices = np.random.permutation(len(nba_geno))
    nba_geno_shuffled = nba_geno[shuffle_indices]
    nba_pheno_shuffled = nba_pheno[shuffle_indices] 
    
    kf = KFold(n_splits = n_splits, shuffle = True, random_state = random_state) 
    for fold_idx, (train_index, test_index) in enumerate(kf.split(nba_geno_shuffled)): 
        print(f"Fold [{fold_idx + 1}/{n_splits}]")
        
        X_train, X_test, y_train, y_test = train_test_split(nba_geno_shuffled, nba_pheno_shuffled, test_size=0.2, random_state=42)
...
class ModelEstimator(BaseEstimator, RegressorMixin): 
    ...
    def fit(self, X, y): 
        ...
        X_train, X_test, y_train, y_test = train_test_split(nba_geno_shuffled, nba_pheno_shuffled, test_size=0.2, random_state= random_state)
    ...
    def score(self, X, y): 
        ...
        #print("Test loader size: ", len(nba_test_loader.dataset))
...
if __name__ == "__main__": 
    #Train Model
    train_with_cv(model, criterion, optimizer, nba_geno, nba_pheno, num_epochs, batch_size, n_splits = 5)
    
    #Save Model 
    torch.save(model.state_dict(), "nba_model_ckpt.ckpt")
