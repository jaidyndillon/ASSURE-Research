...
#Establish percent used to divide datasets
total = 1000  
...
#Create Data Splitter (so that I can run the data through without reaching GPU memory limit)
use_columns = range(6, input_size + 6, 5)

#Load the files
tf_geno = np.loadtxt("C:/Users/Jaidyn/Documents/ASSURE/python/project/repro_traits_model/TF/tf_test.raw", dtype = "float32",
                     skiprows = 1, usecols = use_columns)
tf_pheno = np.loadtxt("C:/Users/Jaidyn/Documents/ASSURE/python/project/repro_traits_model/TF/tf_pheno.csv", dtype = "float32", 
                      skiprows = 1, delimiter = ",", usecols = range(1, 2))
tf_pheno = np.reshape(tf_pheno, (tf_pheno.shape[0], 1)) 

#Scaling and Modifying for Datasets 
tf_geno = StandardScaler().fit_transform(tf_geno)
tf_pheno = StandardScaler().fit_transform(tf_pheno)

#Check Geno and Pheno Shapes
print("tf geno:", tf_geno.shape)
print("tf pheno:", tf_pheno.shape)
...
#Datasets 
tf_train_dataset = GenDataset(tf_geno[0:train_percent,], tf_pheno[0:train_percent, 0])
tf_test_dataset = GenDataset(tf_geno[train_percent:,], tf_pheno[train_percent:, 0])

#Create Test Abs
tf_test_abs = int(len(tf_train_dataset) * 0.8)
tf_train_subset, tf_val_subset = torch.utils.data.random_split(tf_train_dataset, [tf_test_abs, len(tf_train_dataset) - tf_test_abs])

#Check Division of Datasets
#print("Train dataset:", len(tf_train_dataset))
#print("Test dataset:", len(tf_test_dataset))
#print("Train subset:", len(tf_train_subset))
#print("Val subset: ", len(tf_val_subset))

#DataLoaders 
tf_train_loader = torch.utils.data.DataLoader(dataset = tf_train_subset, 
                                              batch_size = batch_size, 
                                              shuffle = True) 
tf_val_loader = torch.utils.data.DataLoader(dataset = tf_val_subset, 
                                         batch_size = batch_size,
                                         shuffle = False)
tf_test_loader = torch.utils.data.DataLoader(dataset = tf_test_dataset, 
                                          batch_size = batch_size, 
                                          shuffle = False)

#Define FlashAttentionLayer
#specific type of encoder layer that improves the efficiency (particularly for large datasets)
...
class TransformerModel(nn.Module): 
    ...
    def forward(self, x):
        ...
        x = self.decoder(x) #gives [batch_size, seq_length, output
...
#Loss and Optimizer 
criterion = nn.MSELoss() #provides direct measure of prediction accuracy
...
#Model Training 
def train(model, tf_train_loader, tf_val_loader, criterion, optimizer, num_epochs): 
    best_val_acc = -1.0 
    patience = 10
    counter = 0 
    all_acc = [] 
    ...
    for epoch in range(num_epochs): 
            ...
            for geno, pheno in tf_train_loader: 
                geno = geno.to(device)
            ...
            with torch.no_grad(): 
                for geno, pheno in tf_val_loader: 
                    geno = geno.to(device) 
                    pheno = pheno.to(device)
...
#Model Testing 
def test(model, tf_test_loader): 
    with torch.no_grad(): 
        outputs_list = [] 
        pheno_list = [] 
        for geno, pheno in tf_test_loader:
            geno = geno.to(device)
            pheno = pheno.to(device)
...
def train_with_cv(model, criterion, optimizer, X, y, num_epochs, batch_size, n_splits = 5): 
    ...
    shuffle_indices = np.random.permutation(len(tf_geno))
    tf_geno_shuffled = tf_geno[shuffle_indices]
    tf_pheno_shuffled = tf_pheno[shuffle_indices] 
    
    kf = KFold(n_splits = n_splits, shuffle = True, random_state = random_state) 
    for fold_idx, (train_index, test_index) in enumerate(kf.split(tf_geno_shuffled)): 
        print(f"Fold [{fold_idx + 1}/{n_splits}]")
        
        X_train, X_test, y_train, y_test = train_test_split(tf_geno_shuffled, tf_pheno_shuffled, test_size=0.2, random_state=42)
...
class ModelEstimator(BaseEstimator, RegressorMixin): 
    ...
    def fit(self, X, y): 
        ...
        X_train, X_test, y_train, y_test = train_test_split(tf_geno_shuffled, tf_pheno_shuffled, test_size=0.2, random_state= random_state)
    ...
    def score(self, X, y): 
        ...
        print("Test loader size: ", len(rsv_test_loader.dataset))
...
if __name__ == "__main__":
    ...
    search.fit(tf_geno, tf_pheno)
    
    print("Best Parameters: ", search.best_params_)
    print("Best Corr Coef: ", search.best_score_)
...
if __name__ == "__main__":    
    #CV Training 
    train_with_cv(model, criterion, optimizer, tf_geno, tf_pheno, num_epochs, batch_size, n_splits = 5)
    
    #Save Model 
    torch.save(model.state_dict(), "tf_model_ckpt.ckpt")
