...
#Load the files
bf_geno = np.loadtxt("C:/Users/Jaidyn/Documents/ASSURE/python/project/repro_traits_model/BF/bf_test.raw", dtype = "float32",
                     skiprows = 1, usecols = use_columns)
bf_pheno = np.loadtxt("C:/Users/Jaidyn/Documents/ASSURE/python/project/repro_traits_model/BF/bf_pheno.csv", dtype = "float32", 
                      skiprows = 1, delimiter = ",", usecols = range(1, 2))
bf_pheno = np.reshape(bf_pheno, (bf_pheno.shape[0], 1)) 

#Scaling and Modifying for Datasets 
bf_geno = StandardScaler().fit_transform(bf_geno)
bf_pheno = StandardScaler().fit_transform(bf_pheno)

#Check Geno and Pheno Shapes
print("bf geno:",bf_geno.shape)
print("bf pheno:", bf_pheno.shape)
...
#Datasets 
bf_train_dataset = GenDataset(bf_geno[0:train_percent,], bf_pheno[0:train_percent, 0])
bf_test_dataset = GenDataset(bf_geno[train_percent:,], bf_pheno[train_percent:, 0])

#Create Test Abs
bf_test_abs = int(len(bf_train_dataset) * 0.8)
bf_train_subset, bf_val_subset = torch.utils.data.random_split(bf_train_dataset, [bf_test_abs, len(bf_train_dataset) - bf_test_abs])

#Check Division of Datasets
#print("Train dataset:", len(bf_train_dataset))
#print("Test dataset:", len(bf_test_dataset))
#print("Train subset:", len(bf_train_subset))
#print("Val subset: ", len(bf_val_subset))

#DataLoaders 
bf_train_loader = torch.utils.data.DataLoader(dataset = bf_train_subset, 
                                              batch_size = batch_size, 
                                              shuffle = True) 
bf_val_loader = torch.utils.data.DataLoader(dataset = bf_val_subset, 
                                         batch_size = batch_size,
                                         shuffle = False)
bf_test_loader = torch.utils.data.DataLoader(dataset = bf_test_dataset, 
                                          batch_size = batch_size, 
                                          shuffle = False)
...
#Model Training 
def train(model, bf_train_loader, bf_val_loader, criterion, optimizer, num_epochs): 
    best_val_acc = -1.0 
    patience = 10
    counter = 0 
    all_acc = [] 
    ...
    for epoch in range(num_epochs): 
            ...
            for geno, pheno in bf_train_loader: 
                geno = geno.to(device)
            ...
            with torch.no_grad(): 
                for geno, pheno in bf_val_loader: 
                    geno = geno.to(device) 
                    pheno = pheno.to(device)
...
#Model Testing 
def test(model,bf_test_loader): 
    with torch.no_grad(): 
        outputs_list = [] 
        pheno_list = [] 
        for geno, pheno in bf_test_loader:
            geno = geno.to(device)
            pheno = pheno.to(device)
...
def train_with_cv(model, criterion, optimizer, X, y, num_epochs, batch_size, n_splits = 5): 
    ...
    shuffle_indices = np.random.permutation(len(bf_geno))
    bf_geno_shuffled = bf_geno[shuffle_indices]
    bf_pheno_shuffled = bf_pheno[shuffle_indices] 
    
    kf = KFold(n_splits = n_splits, shuffle = True, random_state = random_state) 
    for fold_idx, (train_index, test_index) in enumerate(kf.split(bf_geno_shuffled)): 
        print(f"Fold [{fold_idx + 1}/{n_splits}]")
        
        X_train, X_test, y_train, y_test = train_test_split(bf_geno_shuffled, bf_pheno_shuffled, test_size=0.2, random_state=42)
...
class ModelEstimator(BaseEstimator, RegressorMixin): 
    ...
    def fit(self, X, y): 
        ...
        X_train, X_test, y_train, y_test = train_test_split(bf_geno_shuffled, bf_pheno_shuffled, test_size=0.2, random_state= random_state)
...
if __name__ == "__main__":    
    #CV Training 
    train_with_cv(model, criterion, optimizer, bf_geno, bf_pheno, num_epochs, batch_size, n_splits = 5)
    
    #Save Model 
    torch.save(model.state_dict(), "bf_model_ckpt.ckpt")
