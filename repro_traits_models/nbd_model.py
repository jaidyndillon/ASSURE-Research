...
#Establish percent used for train dataset
total = 3290
...
nbd_geno = np.loadtxt("C:/Users/Jaidyn/Documents/ASSURE/python/project/repro_traits_model/NBD/spg_NBD.raw", dtype = "float32",
                      skiprows = 1, usecols = use_columns)
nbd_pheno = np.loadtxt("C:/Users/Jaidyn/Documents/ASSURE/python/project/repro_traits_model/NBD/NBD_pheno.csv", dtype = "float32",
                       skiprows = 1, delimiter = ",", usecols = range(1, 2))
nbd_pheno = np.reshape(nbd_pheno, (nbd_pheno.shape[0], 1))

nbd_geno = StandardScaler().fit_transform(nbd_geno)
nbd_pheno = StandardScaler().fit_transform(nbd_pheno)

#Check Shape of Geno and Pheno
print("nbd geno: ", nbd_geno.shape)
print("nbd pheno: ", nbd_pheno.shape)
...
#Datasets
nbd_train_dataset = GenDataset(nbd_geno[0:train_percent,], nbd_pheno[0:train_percent, 0])
nbd_test_dataset = GenDataset(nbd_geno[train_percent:,], nbd_pheno[train_percent:, 0])

#Create Test Abs
nbd_test_abs = int(len(nbd_train_dataset) * 0.8)
nbd_train_subset, nbd_val_subset = torch.utils.data.random_split(nbd_train_dataset, [nbd_test_abs, len(nbd_train_dataset) - nbd_test_abs])
...
#DataLoader
nbd_train_loader = torch.utils.data.DataLoader(dataset = nbd_train_subset, 
                                              batch_size = batch_size, 
                                              shuffle = True) 
nbd_val_loader = torch.utils.data.DataLoader(dataset = nbd_val_subset, 
                                         batch_size = batch_size,
                                         shuffle = False)
nbd_test_loader = torch.utils.data.DataLoader(dataset = nbd_test_dataset, 
                                          batch_size = batch_size, 
                                          shuffle = False)
...
#Model Training 
def train(model, nbd_train_loader, nbd_val_loader, criterion, optimizer, num_epochs): 
    best_val_acc = -1.0 
    patience = 10
    counter = 0 
    all_acc = [] 
    ...
    for epoch in range(num_epochs): 
            ...
            for geno, pheno in nbd_train_loader: 
                geno = geno.to(device)
            ...
            with torch.no_grad(): 
                for geno, pheno in nbd_val_loader: 
                    geno = geno.to(device) 
                    pheno = pheno.to(device)
...
#Model Testing 
def test(model, nbd_test_loader): 
    with torch.no_grad(): 
        outputs_list = [] 
        pheno_list = [] 
        for geno, pheno in nbd_test_loader:
            geno = geno.to(device)
            pheno = pheno.to(device)
...
def train_with_cv(model, criterion, optimizer, X, y, num_epochs, batch_size, n_splits = 5): 
    ...
    shuffle_indices = np.random.permutation(len(nbd_geno))
    nbd_geno_shuffled = nbd_geno[shuffle_indices]
    nbd_pheno_shuffled = nbd_pheno[shuffle_indices] 
    
    kf = KFold(n_splits = n_splits, shuffle = True, random_state = random_state) 
    for fold_idx, (train_index, test_index) in enumerate(kf.split(nbd_geno_shuffled)): 
        print(f"Fold [{fold_idx + 1}/{n_splits}]")
        
        X_train, X_test, y_train, y_test = train_test_split(nbd_geno_shuffled, nbd_pheno_shuffled, test_size=0.2, random_state=42)
...
class ModelEstimator(BaseEstimator, RegressorMixin): 
    ...
    def fit(self, X, y): 
        ...
        X_train, X_test, y_train, y_test = train_test_split(nbd_geno_shuffled, nbd_pheno_shuffled, test_size=0.2, random_state= random_state)
...
if __name__ == "__main__": 
    #Train Model
    train_with_cv(model, criterion, optimizer, nbd_geno, nbd_pheno, num_epochs, batch_size, n_splits = 5)
    
    #Save Model 
    torch.save(model.state_dict(), "nbd_model_ckpt.ckpt")
